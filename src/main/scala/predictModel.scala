...
object predictModel {
  def main(args: Array[String]): Unit = {
    ...
    var result = model.transform(vectorDF).toDF()


    result = result.select('memberId.as("id"),when('predict === 0,"很低")
    .when('predict === 1,"低")
    .when('predict === 2,"中下")
    .when('predict === 3,"中")
    .when('predict === 4,"中上")
    ...
    def catalogWrite =
      s"""{
         ...
         |"columns":{
         |"id":{"cf":"rowkey", "col":"id", "type":"string"},
         |"powerOfConsumption":{"cf":"Commercial", "col":"powerOfConsumption", "type":"string"}
         |}
         |}""".stripMargin
