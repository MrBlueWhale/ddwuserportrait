...
object cateFeaturedModel {
  def main(args: Array[String]): Unit = {
    ...
    val cate = spark.sql(
      """
        |select memberId, cateId from ordersDF join goodsDF on ordersDF.relationOrderSn = goodsDF.cOrderSn
      """.stripMargin)

    cate.show(100,false)
    val ratingDF = cate.select(
      'memberId.as("userId").cast(DataTypes.IntegerType),
      'cateId.cast(DataTypes.IntegerType)
    ).filter('cateId.isNotNull)
      .groupBy('userId, 'cateId)
...
*/
    ...
    val result: DataFrame = model.recommendForAllUsers(5)
      .withColumn("favorCates", predict2StringFunc('recommendations))
      .withColumnRenamed("userId", "id")
      .drop('recommendations)
      .select('id.cast(LongType), 'favorCates)
    ...
    val resultDf = result.withColumn("arrayCol",split(col("favorCates"),",")).withColumn("expCol",explode(col("arrayCol"))).select('id as "memberId", 'expCol as "cateId")

    val resultDuplication = resultDf.except(cate).select('memberId as "id", 'cateId as "favorCates")
...
//    val Df1 = spark.sql(
//      """
//        |select memberId,concat_ws(',',collect_set(cateId)) as favorCates from test group by memberId
//      """.stripMargin)
//
//    Df1.show(100,false)
    ...
    def recommendationCatalog =
      s"""{
         ...
         |     "favorCates":{"cf":"Recommendation", "col":"favorCates", "type":"string"}
         |   }
         |}""".stripMargin
